...
void Oled_WriteChar1x(char letter, unsigned char page, unsigned char column, BOOL inverse);
...
*
********************************************************************/
static Emode mode = NULL;

void SetMode(Emode mod)
{
		mode = mod;
}
/*******************************************************************/
...
void Oled_Init(void){
    ...
    oledWR = 0;
    oledWR_TRIS = 0;  // set WR bit as output
    oledRD = 0;
    oledRD_TRIS = 0;  // set RD bit as output
    oledCS = 1;
    oledCS_TRIS = 0;  // set CS bit as output
    oledD_C	= 0;
    oledD_C_TRIS = 0; // set D/C bit as output
...
void Oled_PutROMString(rom unsigned char *ptr,unsigned char line, unsigned char col)
{
	unsigned char i;
	unsigned char page;
...
void Oled_PutString(unsigned char *ptr,unsigned char line, unsigned char col, BOOL inverse)
{
	...
	while(*ptr)
	{
		Oled_WriteChar1x(*ptr,page,i, inverse);
		ptr++;
		i+=6;
	}
}
...
static void WriteCommand(BYTE cmd)
{
    ...
    LATD  = cmd;	// set the command as port D data 
    oledRD = 1;		// clear the read signal
    oledWR = 1;		// clear the write signal
    oledD_C	= 0;	// set the D/C bit as command (0)
    oledCS = 0;		// set the CS bit as 0 -> select the display
    oledWR = 0;		// enable the write
    oledWR = 1;		// disable the write
    oledCS = 1;		// clear the CS
    TRISD = 0xFF;	// set port D as input
    for(i=0;i<10;i++){}	// wait for the effect to take place
}
...
{
    TRISD = 0x00;	// set port D as output
    LATD  = data;	// set the data as port D data 
    oledRD = 1;		// clear the read signal
    oledWR = 1;		// clear the write signal
    oledD_C	= 1;	// set the D/C bit as data (1)
    oledCS = 0;		// set the CS bit as 0 -> select the display
    oledWR = 0;		// enable the write
    oledWR = 1;		// disable the write
    oledCS = 1;		// clear the CS
...
void Oled_WriteChar1x(char letter, unsigned char page, unsigned char column, BOOL inverse)
{
	...
	{
		WriteData(g_AlienFont[letter][0]);	// Write first column
		WriteData(g_AlienFont[letter][1]);	// Write second column
		WriteData(g_AlienFont[letter][2]);	// Write third column
		WriteData(g_AlienFont[letter][3]);	// Write fourth column
		WriteData(g_AlienFont[letter][4]);	// Write fifth column
...
ROM BYTE g_AlienFont[95][5] =
{
    ...
    { 0xc8, 0x34, 0x23, 0x34, 0xc8 }, // A - Robot1 OpenLegs    0x41
    { 0x02, 0x3c, 0xcb, 0x3c, 0x02 }, // B - Robot1 CloseLegs   0x42
	{ 0x88, 0x4f, 0x39, 0x4f, 0x88 }, // C - Robot2 OpenLegs    0x43
    { 0x04, 0x8f, 0x79, 0x8f, 0x04 }, // E - Robot2 CloseLegs   0x45
	{ 0x04, 0x8a, 0xff, 0x8a, 0x04 }, // D - Spaceship1  		0x44
	{ 0x4c, 0x74, 0xce, 0x74, 0x4c }, // F - Spaceship2  		0x46
	{ 0x5a, 0x00, 0xff, 0x00, 0x5a }, // G - Hit   				0x47
	{ 0x00, 0x00, 0x3c, 0x00, 0x00 }, // H - Bullet1  			0x48
	{ 0x00, 0x00, 0x18, 0x00, 0x00 }, // I - Bullet2    		0x48
    { 0x20, 0x40, 0x41, 0x3f, 0x01 }, // J   0x4A
    { 0x7f, 0x08, 0x14, 0x22, 0x41 }, // K   0x4B
    { 0x7f, 0x40, 0x40, 0x40, 0x40 }, // L   0x4C
   	{ 0xff, 0xff, 0xff, 0xff, 0xff }, // M   New M 0x4D
    { 0x80, 0x80, 0x80, 0x80, 0x80 }, // N   New N 0x4E
    { 0x80, 0x80, 0x80, 0x80, 0x80 }, // O   New O 0x4F
    { 0x80, 0x80, 0xc0, 0x20, 0x00 }, // P   New P 0X50
    { 0xf8, 0x04, 0x02, 0x01, 0x00 }, // Q   New Q low row 0X51
    { 0x00, 0x00, 0x7f, 0x9d, 0x9f }, // R   New R low row 0X52
    { 0x00, 0x9f, 0x9d, 0x7f, 0x00 }, // S   New S low row 0X53
    { 0x00, 0x01, 0x02, 0x04, 0xf8 }, // T   New T low row 0X54
